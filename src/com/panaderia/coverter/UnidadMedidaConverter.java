...
@ManagedBean(name = "unidadConverter")
@RequestScoped
public class UnidadMedidaConverter implements Converter {
	...
	private static final Logger logger = Logger.getLogger(UnidadMedidaConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent component, String value) {
		try {
			if (value != null && value.trim().length() > 0) {
				return getiBriocheServicio().obtenerUnidadXId(Integer.valueOf(value));
			} else {
				return null;
			}
		} catch (Exception e) {
	...
	public String getAsString(FacesContext context, UIComponent component, Object value) {
		if (value != null) {
			return String.valueOf(((CatUnidadMedida) value).getId());
		} else {
			return null;
		}
	}
