...
@ManagedBean(name = "tipoPanConverter")
@RequestScoped
public class TipoPanConverter implements Converter {

	@ManagedProperty(value = "#{BriocheServicio}")
	IBriocheServicio iBriocheServicio;

	private static final Logger logger = Logger.getLogger(TipoPanConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent component, String value) {
		try {
			if (value != null && value.trim().length() > 0) {
				return getiBriocheServicio().obtenerTipoPanXCodigo(value);
			} else {
				return null;
			}
		} catch (Exception e) {
	...
	public String getAsString(FacesContext context, UIComponent component, Object value) {
		if (value != null) {
			return String.valueOf(((CatTipoPan) value).getCodigo());
		} else {
			return null;
		}
	}
